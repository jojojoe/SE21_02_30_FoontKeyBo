...
enum AdjustKey: String {
    case AdjustKeyAppToken = "v5b8ox2ynd34"
    case AdjustKeyAppLaunch = "ozd8wf"
    case AdjustKeyAppCoinsBuy = "l4n2du"
...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    var window: UIWindow?
    var mainVC: MAinHomeVC = MAinHomeVC()
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        //
        //
        let contentUrl: URL? = launchOptions?[UIApplication.LaunchOptionsKey.url] as? URL
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now()+0.5) {
            [weak self] in
            guard let `self` = self else {return}
            self.checkShowStoreVC(urlString: contentUrl?.absoluteString ?? "")
        }
...
//        com.keyinsfont.superone
        
        NotificationCenter.default.post(name: .water,
                                        object: [
                                            HightLigtingHelper.default.debugBundleIdentifier = "com.keyinsfont.superone",
...
extension AppDelegate {
    func checkShowStoreVC(urlString: String) {
        if urlString == "KeyBymKeyBoard://\("Store")" {
            
            FKbCoinMana.default.loadCoinCountFromGroup()
            
            if self.mainVC.visibleVC?.className.contains("KFSToreVC") ?? false {
                
            } else {
                self.mainVC.present(KFSToreVC(), animated: true, completion: nil)
            }
            self.mainVC.refreshTopCoin()
        }
    }
